...
void i2c_configure_slave(unsigned char addr) {
    ...
    // set the address
    SSPADD = addr;
    //char tempreg = SSPBUF; //clear it
    //OpenI2C(SLAVE_7,SLEW_OFF); // replaced w/ code below
    SSPSTAT = 0x0;
    SSPCON1 = 0x0;
    SSPCON2 = 0x0;
   // SSPCON2bits.GCEN = 1; //should respond to any address
    SSPCON1 |= 0x0E; // enable Slave 7-bit w/ start/stop interrupts
   //  SSPCON1 |= 0x06;   //enable Slave 7-bit w/o start/stop interrupts
    SSPSTAT |= SLEW_OFF;
#ifdef I2C_V3
    I2C1_SCL = 1;
    I2C1_SDA = 1;
