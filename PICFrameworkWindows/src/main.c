...
void main(void) {
...
#else
    OSCCON = 0x82; // see datasheeet (ben: primary oscillator at 12MHZ)
    OSCTUNEbits.PLLEN = 0; // Makes the clock exceed the PIC's rated speed if the PLL is on (ben: turned off so not up to 40MHZ)
#endif
   ...
   // TMR1L = 62;             // preset for timer1 LSB register  (for .00015s ints)
